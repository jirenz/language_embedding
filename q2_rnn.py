...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        for i in xrange(max_length):
            if i < len(sentence):
                new_s.append(sentence[i])
                new_l.append(labels[i])
                new_m.append(True)
            else:
                new_s.append(zero_vector)
                new_l.append(zero_label)
                new_m.append(False)
        ret.append((new_s, new_l, new_m))
    ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        indexed_embeddings = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder)
        embeddings = tf.reshape(indexed_embeddings, [-1, self.max_length, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        h = tf.constant(0, shape=[1, self.config.hidden_size], dtype=tf.float32)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                reshaped_x = tf.transpose(x, perm=[1,0,2])[time_step]
                # reshaped_x = tf.squeeze(tf.slice(x, [0, time_step, 0], [-1, 1, -1]))
                o, h = cell(reshaped_x, h)
                o_drop = tf.nn.dropout(o, self.dropout_placeholder)
                preds.append(tf.add(tf.matmul(o_drop, U), b2))
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        before_mask = tf.nn.sparse_softmax_cross_entropy_with_logits(preds, self.labels_placeholder)
        masked = tf.boolean_mask(before_mask, self.mask_placeholder)
